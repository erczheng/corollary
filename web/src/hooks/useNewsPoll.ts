 ...
 * stand in for three different Alpaca mechanisms. The websockets are capped
 * per asset class — 30 equity symbols, 200 option quotes — and spent on
 * open positions, snapshot requests are bounded by a request budget and
 * cover the chains, and the news endpoint is neither — it is not per-symbol
 * at all, and its cadence is set by how often headlines are actually
 * published rather than by how fast a price moves.
 *
