...
The safeguard was never the switch's location. Cash is entered through the confirm dialog above no matter where the toggle is rendered, it never survives a restart, and every cold start comes up in Paper. That confirm names **buying power first, then cash** — a cash account has no margin and so can spend less than its balance, and quoting cash alone in the dialog that precedes real trading would overstate what can actually be deployed. The difference is stated rather than left to be noticed, and stated without being attributed: see §8.6 on why its cause is not available. Pages that only *display* account context and don't scope their whole contents to it (News, Markets, Research, Settings) get the badge and no switch.

The rule is what decides this, not the list. Account was added to the first group when the page was built: every figure on it — cash, buying power, options buying power, its transfer ledger — belongs to exactly one account, which makes it the most completely account-scoped page in the app. Settings stays in the second group: it displays the badge and scopes nothing to it, apart from the equity figure a risk-limit confirm quotes.
...
| Real-time equities + options | **Alpaca Basic** (free), upgrading at Phase 4 | Equities: IEX for live, SIP free for historical >15 min. Options `indicative` feed. 200 req/min. **Two websocket budgets, not one: 30 equity symbols and 200 option quotes.** |
...
**Plan upgrade trigger.** Basic carries Phases 1–3: the shell, most read-only views, and all backtesting, since everything older than 15 minutes is available on every feed. **Algo Trader Plus ($99/mo)** becomes necessary at **Phase 4** — pulled forward from Phase 6 on 2026-09-14; the reasoning is decision 19 of the Phase 2 design spec, not restated here. Subscribe then, not before.

**Stream budget.** Basic carries **two** websocket budgets, not one shared pool of thirty: **30 equity symbols** and **200 option quotes**. Every option contract is its own symbol, so the 200 is the budget a position stream spends and the 30 is spent only on underlyings. The live stream is scoped to open positions plus recommended trades; Markets-page chains and scanner universe scans run on polled snapshots. Algo Trader Plus lifts the equity stream to unlimited and the option stream to **1000 quotes** — a higher ceiling, not the absence of one. The polled path should remain the default regardless, so the app degrades gracefully rather than depending on the subscription.
...
- **Options chains:** symbol, strike, expiration, last, change, change %, bid, ask. Filter by underlying, volume, top gainers, top losers, highest IV, highest open interest. Paginated.
  ...
  - The honest cost of deriving: greeks computed from a 15-minute-delayed mid are *delayed* greeks. Correct method, stale inputs. Acceptable in a column, unacceptable for sizing — which is the boundary at which the plan gets bought, before Phase 4 rather than before Phase 2.
...
- ~~**Engine and API: one process or two.**~~ **Resolved 2026-09-14 — one process.** `uv run python -m uvicorn corollary.api:app` is the whole app; the scheduler, the streams and `EngineRuntime`'s watchdog are asyncio tasks started in the FastAPI lifespan. SQLite gets a single writer for free, which is what the two-process split would have needed WAL-plus-single-writer discipline to buy — and `database is locked` is still handled explicitly rather than assumed away, since the busy timeout is now a stated 5000 ms rather than an inherited default. More importantly, the code that detects a dropped Alpaca connection is the same process that answers `/api/engine/state`, so there is no window in which the API reports *running* because it has not heard otherwise — which is the one failure §4's dead-man's switch cannot tolerate. `corollary/engine/` and `corollary/api/` stay separate modules, so the two-process split §3 draws remains a deployment change rather than a refactor. Rejected: two processes — real crash isolation, but Basic allows one websocket per account, so the API would have to proxy quotes through the engine; an IPC channel is required either way and the DB contention is not. Side effect worth naming: `uvicorn --reload` restarts the engine on every edit, landing you in halted-Paper, which under rules 5 and 9 is the correct posture and not a cost.
